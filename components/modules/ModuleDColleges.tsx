...
export function ModuleDColleges() {
    ...
    const getProbability = (collegeComp: string) => {
        if (profile.academicStanding === "High Achiever") return { val: 85, color: "text-emerald-600 dark:text-emerald-400", label: "Strong Chance" };
        if (profile.academicStanding === "Average" && collegeComp.includes("Brutal")) return { val: 15, color: "text-destructive", label: "Very Low" };
        if (profile.academicStanding === "Average" && collegeComp.includes("High")) return { val: 40, color: "text-amber-600 dark:text-amber-400", label: "Difficult" };
        return { val: 60, color: "text-primary", label: "Possible" };
    };
    ...
    return (
        <div className="space-y-12 max-w-6xl mx-auto py-8 px-4">
            ...
            <section className="text-center space-y-4">
                <Badge variant="outline" className="text-purple-600 dark:text-purple-400 border-purple-500/30">Module D: Undergraduate Choices (Chennai)</Badge>
                <h1 className="text-3xl md:text-5xl font-heading font-bold text-foreground">
                    The College Hunt.
                </h1>
                <p className="text-muted-foreground max-w-xl mx-auto">
                    You are now ~17 years old. You need a Bachelor's degree.
                    In Chennai, the college "brand" matters for your Master's application later.
                </p>
            </section>
                        ...
                        >
                            <Card className="h-full bg-card border-border hover:border-purple-500/50 transition-all flex flex-col">
                                <CardHeader className="pb-2">
                                    ...
                                    <CardTitle className="text-lg text-foreground leading-tight">{college.name}</CardTitle>
                                </CardHeader>
                                <CardContent className="space-y-4 flex-1 text-sm text-muted-foreground">
                                    ...
                                    <div className="space-y-1">
                                        <p className="text-xs text-muted-foreground uppercase tracking-widest font-bold">Key Courses</p>
                                        <ul className="list-disc pl-4 text-muted-foreground/80">
                                            {college.courses.slice(0, 2).map(c => <li key={c}>{c}</li>)}
                                        </ul>
                                    </div>
                                    <div className="flex justify-between items-center border-t border-border pt-3">
                                        <span className="text-muted-foreground">Fees/Yr</span>
            ...
            <section className="bg-amber-500/10 dark:bg-amber-950/20 p-6 rounded-xl border border-amber-500/20 flex gap-4 items-start">
                ...
                <div>
                    <h3 className="font-bold text-amber-700 dark:text-amber-200 mb-1">Warning: Marks vs Reality</h3>
                    <p className="text-sm text-amber-700/80 dark:text-amber-200/80">
                        Admission to colleges like Loyola or MCC is incredibly competitive.
                        If you don't have 95%+ in Grade 12 (CBSE/State), you need a backup plan.
                    </p>
                </div>
            </section>
